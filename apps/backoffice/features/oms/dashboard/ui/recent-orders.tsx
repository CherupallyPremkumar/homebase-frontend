    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          Recent Orders
          <Link href="/oms/orders" className="text-sm font-normal text-primary hover:underline">
            View all
          </Link>
        </CardTitle>
      </CardHeader>
        ...
        {recentOrders?.length ? (
          <div className="space-y-3">
            {recentOrders.slice(0, 10).map((order) => (
              <Link key={order.id} href={`/oms/orders/${order.id}`} className="flex items-center justify-between rounded p-2 text-sm hover:bg-gray-50">
                <div>
